...
import jline.lang.controllers.PricingController;
...
public class RevenueMetric extends Metric<Double,Double>{
    protected PricingController pricingController;
    public RevenueMetric(PricingController pricingController, int nodeIdx, int classIdx, int nServers, boolean record) {
        super("Revenue", 0.0, 0.0, record, nodeIdx, classIdx, nServers);
        this.pricingController = pricingController;
    }
    public RevenueMetric(PricingController pricingController, int nodeIdx, int classIdx, int nServers) {
        this(pricingController, nodeIdx, classIdx, nServers, false);
    }
    ...
    public void fromEvent (double t, Event e) {
        if (e instanceof DepartureEvent) {
            ...
            if ((de.getNode().getStatefulIdx() == this.nodeIdx) &&
                ...
                double totalRevenue = this.metricValue * (this.time-this.cutoffTime);
                totalRevenue += this.pricingController.getPrice();
                if (this.record) {
                ...
                this.time = t;
                this.metricValue = totalRevenue/(this.time-this.cutoffTime);
            }
        }
    }
    ...
    public void fromEvent(double t, Event e, int n) {
        if (e instanceof DepartureEvent) {
            ...
            if ((de.getNode().getStatefulIdx() == this.nodeIdx) &&
                ...
                double tDelta = t - this.time;
                double totalRevenue = this.metricValue * (this.time-this.cutoffTime);
                totalRevenue += n*this.pricingController.getPrice();
